<script setup>
    ...
    const { obtenerIdeaPorId, editarIdea, loading, error } = useIdeas()
    ...
    onMounted(async () => {
        const idea = await obtenerIdeaPorId(ideaId)
    ...
    const handleSubmit = async (data) => {
        try {
            await editarIdea(ideaId, data)

            toast.showToast('Idea actualizada correctamente', 'success')

            router.push(ROUTES.GENERAL.ENCUESTAS.MIS_IDEAS.LISTA)

        } catch (err) {
            console.error(err)
            toast.showToast('No se pudo actualizar la idea', 'error')
        }
    }
</script>
